...
function copyUniAppBaseComponents(): void {
  console.log('📦 Generating UniApp VuIcon.vue with inline iconData...')

  // Read icons-data.js content
  const iconsDataPath = path.join(DIST, 'icons-data.js')
  const iconsDataContent = fs.readFileSync(iconsDataPath, 'utf-8')

  // Generate VuIcon.vue with inline iconData (no external imports)
  const vuIconContent = `<script>
// Inline iconData to avoid UniApp module resolution issues
${iconsDataContent.replace('export const iconData', 'const iconData')}
...
`
  ...
  fs.writeFileSync(path.join(UNIAPP_DIR, 'VuIcon.vue'), vuIconContent)

  console.log('✅ UniApp VuIcon.vue generated with inline iconData')
}
